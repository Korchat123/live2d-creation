...
`P0-A` is an executable **geometry-only Gate A candidate** for `standard-bust-v1/spec-0.8.0`. `CHARACTER_BIBLE.md` is the governing reviewed contract; its explicit parameter parity table is checked against `src/spec.js`, which supplies those exact values to controls, geometry, validation, evidence states, and tests. The app visibly exposes the same version. The candidate contains no decorative character artwork, hair/outfit assets, generated finished image, or rig.
